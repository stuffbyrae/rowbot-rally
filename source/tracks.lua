...
function tracks:init(...)
    ...
    vars = {
        ...
        boat_anim = gfx.animator.new(3500, 0, 10, pd.easingFunctions.inOutQuad),
        wave_anim = gfx.animator.new(1000, 0, -50)
    }
    
    vars.besttime = save.t1
    vars.boat_anim.reverses = true
    vars.boat_anim.repeatCount = -1
    ...
    function boat:update()
        self:moveTo(100, 170+vars.boat_anim:currentValue())
    end
    ...
    function wave:init()
        ...
        self:setZIndex(3)
        local img = gfx.image.new(450, 31)
        gfx.pushContext(img)
            assets.img_wave:drawTiled(0, 0, 450, 31)
        gfx.popContext()
        self:setImage(img)
        local img = nil
        self:setCenter(0, 1)
    ...
    function wave:update()
        self:moveTo(vars.wave_anim:currentValue(), 240)
    end
...
function tracks:update()
    if pd.buttonJustPressed('a') then
        if vars.stage_transitioning == false then
            scenemanager:transitionsceneoneway(race, vars.current_menu_item, "tt", vars.arg_boat, false)
        end
    end
